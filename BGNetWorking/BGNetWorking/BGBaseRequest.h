...
FOUNDATION_EXPORT NSString *const BGRequestValidationErrorDomain;

NS_ENUM(NSInteger) {
    BGRequestValidationErrorInvalidStatusCode = -8,
    BGRequestValidationErrorInvalidJSONFormat = -9,
    };
    
    ///  HTTP请求方式
    typedef NS_ENUM(NSInteger, BGRequestMethod) {
        BGRequestMethodGET = 0,
        BGRequestMethodPOST,
        BGRequestMethodHEAD,
        BGRequestMethodPUT,
        BGRequestMethodDELETE,
        BGRequestMethodPATCH,
    };
    
    ///  Request serializer type.
    typedef NS_ENUM(NSInteger, BGRequestSerializerType) {
        BGRequestSerializerTypeHTTP = 0,
        BGRequestSerializerTypeJSON,
    };
    
    ///  Response serializer type, which determines response serialization process and
    ///  the type of `responseObject`.
    typedef NS_ENUM(NSInteger, BGResponseSerializerType) {
        /// NSData type
        BGResponseSerializerTypeHTTP,
        /// JSON object type
        BGResponseSerializerTypeJSON,
        /// NSXMLParser type
        BGResponseSerializerTypeXMLParser,
    };
    
    ///  Request priority
    typedef NS_ENUM(NSInteger, BGRequestPriority) {
        BGRequestPriorityLow = -4L,
        BGRequestPriorityDefault = 0,
        BGRequestPriorityHigh = 4,
    };
    ...
    typedef void(^BGRequestCompletionBlock)(__kindof BGBaseRequest *request);
    
    ///  The BGRequestDelegate protocol defines several optional methods you can use
    ///  to receive network-related messages. All the delegate methods will be called
    ///  on the main queue.
    @protocol BGRequestDelegate <NSObject>
    
    @optional
    ///  Tell the delegate that the request has finished successfully.
    ///
    ///  @param request The corresponding request.
    - (void)requestFinished:(__kindof BGBaseRequest *)request;
    
    ///  Tell the delegate that the request has failed.
    ///
    ///  @param request The corresponding request.
    - (void)requestFailed:(__kindof BGBaseRequest *)request;
    
    @end
    
    ///  The BGRequestAccessory protocol defines several optional methods that can be
    ...
    @protocol BGRequestAccessory <NSObject>
    ...
    ///  BGBaseRequest is the abstract class of network request. It provides many options
    ///  for constructing request. It's the base class of `BGRequest`.
    @interface BGBaseRequest : NSObject
...
///  `BGResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `BGResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<BGRequestDelegate> delegate;
...
@property (nonatomic, copy, nullable) BGRequestCompletionBlock successCompletionBlock;

///  The failure callback. Note if this value is not nil and `requestFailed` delegate method is
///  also implemented, both will be executed but delegate method is first called. This block
///  will be called on the main queue.
@property (nonatomic, copy, nullable) BGRequestCompletionBlock failureCompletionBlock;
...
///  The priority of the request. Effective only on iOS 8+. Default is `BGRequestPriorityDefault`.
@property (nonatomic) BGRequestPriority requestPriority;

///  Set completion callbacks
- (void)setCompletionBlockWithSuccess:(nullable BGRequestCompletionBlock)success
                              failure:(nullable BGRequestCompletionBlock)failure;
...
- (void)startWithCompletionBlockWithSuccess:(nullable BGRequestCompletionBlock)success
                                    failure:(nullable BGRequestCompletionBlock)failure;
...
///  HTTP request method.
- (BGRequestMethod)requestMethod;

///  Request serializer type.
- (BGRequestSerializerType)requestSerializerType;

///  Response serializer type. See also `responseObject`.
- (BGResponseSerializerType)responseSerializerType;
