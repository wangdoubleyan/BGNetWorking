...
void BGLog(NSString *format, ...) {
#ifdef DEBUG
    if (![BGNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        BGLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation BGBaseRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<BGRequestAccessory> accessory in self.requestAccessories) {
...
@implementation BGBatchRequest (RequestAccessory)
...
@implementation BGChainRequest (RequestAccessory)
